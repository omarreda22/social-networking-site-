...
const formBtnLike = document.querySelectorAll('.form-btn-like');
...
//         // submitAction[i].click();
//         // formBtnLike.addEventListener('submit', (e)=>{
//         //     e.preventDefault();
//         //     console.log('submit')
//         // })
//         // console.log(formBtnLike[i])
//     });
// }
...
for (let i = 0; i < formBtnLike.length; i++){
    formBtnLike[i].addEventListener('submit', (e)=>{
            e.preventDefault();
            
            const form = new FormData();
            form.append('csrfmiddlewaretoken', csrf[0].value);
            // form.append('id', submitAction[i].value)
            // console.log(submitAction[i].value)
            url = `${submitAction[i].value}/like_unlike` 
            $.ajax({
                type: 'POST',
                url: url,
                data:form,
...
const imageForm = document.getElementById('id_image');
const contentForm = document.getElementById('id_content');
const formCSRF = document.getElementsByName('csrfmiddlewaretoken');

const openImage = document.getElementById('open-image');
const allPosts = document.getElementById('all-posts');
const imageClose = document.getElementById('close-form')
...
imageForm.addEventListener('change', (e)=>{
    e.preventDefault();
    const openImageIn = imageForm.files[0];
    const createURl = URL.createObjectURL(openImageIn);
    openImage.innerHTML = `<img src="${createURl}" width="50%">`
})
...
postForm.addEventListener('submit', (e)=>{
    ...
    form.append('image', imageForm.files[0]);
    form.append('content', contentForm.value);
    // console.log(formCSRF[0].value)
    $.ajax({
        type:'POST',
        url: 'new_post/',
        ...
        success: function(rep){
            const newPostHtml = `
            <div class="card text-center posts-body mb-3 " style="width:80%; margin:auto;">
                <div class="card-header text-left">
                    <img src="${rep.user_image}"  class="rounded-circle" style="width:45px"alt="Avatar" />
                    <span  class='username-phr'>${rep.user_username}</span>
                ...
                <div class="card-footer">
                    <!-- Like Button -->
                    <form class='form-btn-like'>
                    <input type="hidden" name="csrfmiddlewaretoken" value="TGIJm1wErtAqnFFzsIc40dIkE5951NqIQd8sxT2j5atn6NitSsjH8nPezFY0apcH">
                    <a id='btn-like' href="" class="paw-button">
                        <div class="text">
                            <svg>
                    ...
                    <input type="submit" value="${rep.post_id}" style='display:none' id="submit-action">
                    </form>
...
const commentForm = document.querySelectorAll('.comment-btn-form');
const commentCSRF = document.getElementsByName('csrfmiddlewaretoken');

// console.log(commentCSRF)
// for(let i = 0 ; i<commentCSRF.length; i++){
//     console.log(commentCSRF[i].value)
// }

for(let i=0; i<commentForm.length; i++){
    commentForm[i].addEventListener('submit', (e)=>{
        e.preventDefault();
        const postIdCommnet = commentForm[i].classList[1]
        const CommentBody = commentForm[i].childNodes[1].childNodes[1].value
        const csrf = commentCSRF[i].value
        
        const form = new FormData();
        form.append('csrfmiddlewaretoken', csrf)
        form.append('body', CommentBody)
        
        const newCommentArea = document.getElementById(`new-comment-${postIdCommnet}`)
        const closeBtn = document.getElementById(`close-button-${postIdCommnet}`)
        const commentCount = document.getElementById(`num-comments-${postIdCommnet}`)
        const newCount = Number(commentCount.textContent) + 1
        const newCommentBody = commentForm[i].childNodes[1].childNodes[1]
        
        $.ajax({
            type:'POST',
            url: `new_comment/${postIdCommnet}`,
            data: form,
            success: function(respones){
                const newCommentBodyHtml = `
                <div class="card p-3 m-2">
                <div class="row" >
                    <div class="col-6  text-left">
                        <img src="${respones.avatar}" width="30" class="rounded-circle mr-2">
                        <small class="font-weight-bold text-primary">${respones.user}</small>
                    </div>
                    <div class="col-6 text-right">
                        <p style='font-size:12px'>Just now</p>
                    </div>
                </div>
                
                <p class="text-left">${respones.body}</p>
                </div>
                `                
                newCommentArea.insertAdjacentHTML('afterbegin', newCommentBodyHtml);
                commentCount.innerHTML = newCount
                newCommentBody.value = ''
                alert('success', 'Added New Comment')
                closeBtn.click();

            },
            error: function(error){
                newCommentBody.value = ''
                alert('danger', 'Some Thing Wrong')
                closeBtn.click();
            },
            cache: false,
            contentType: false,
...
const editCommentForm = document.querySelectorAll(".form-edit-comment");

for(let i=0; i<editCommentForm.length; i++){
    editCommentForm[i].addEventListener('submit', (e)=>{
        e.preventDefault();
        const commentId = editCommentForm[i].classList[1]
        const textareaComment = document.getElementById(`comment-body-${commentId}`)
        const newComment = textareaComment.value
        const csrf = commentCSRF[i].value

        const form = new FormData();
        form.append('csrfmiddlewaretoken', csrf)
        form.append('body', newComment)

        const newHtml = document.querySelector(`.comment-body-${commentId}`);
        const closeWindod = document.querySelector(`.update-comment-cloes-${commentId}`)
...
const deleteComment = document.querySelectorAll('.delete-comment-form');

for(let i=0; i<deleteComment.length; i++){
    deleteComment[i].addEventListener('submit', (e)=>{
        e.preventDefault();
        const deleteCommentId = deleteComment[i].classList[1]
        ...
        const commentCard = document.querySelector(`.comment-card-${deleteCommentId}`)
        
        const deleteCommentPostId = deleteComment[i].classList[2]
        const count = document.getElementById(`num-comments-${deleteCommentPostId}`)
        const newCount = Number(count.textContent) - 1;

        const closeWindow = document.querySelector(`.delete-comment-close-${deleteCommentId}`)
        
        $.ajax({
            type:'POST',
            url: `delete_comment/${deleteCommentId}`,
            data: form,
            success: function(rep){
                commentCard.remove()
                // commentCard.style.background = 'none';
                // commentCard.style.border = 'none';
...
for(let i=0; i<showHide.length; i++){
    showHide[i].addEventListener('click', ()=>{
        const contentValue = showHide[i].textContent
        const postId= showHide[i].classList[1];
        const comments = document.getElementById(`comments-box-${postId}`)
        if (contentValue == 'show'){
            showHide[i].textContent = 'hide'
        }
        if (contentValue == 'hide'){
            showHide[i].textContent = 'show'
        }
        
        comments.classList.toggle('display-comments-box')
